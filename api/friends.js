...
router.post('/request', async (req, res) => {
  try {
    ...
    if (pendingRequest) {
      if (
        Number(pendingRequest.sender_id) === receiverId
      ) {
        return res.status(409).json({
          success: false,
  ...
  try {
    ...
    if (senderIds.length > 0) {
      const {
        ...
        .from('users')
        .select(
          'id,name,email,avatar,rating,verified'
        )
        .in('id', senderIds);

      if (usersError) {
        console.error(
          'Error obteniendo remitentes:',
          usersError
        );
  ...
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({
        success: false,
        error:
          'No se pudieron obtener las solicitudes enviadas'
      });
    }
    ...
    if (receiverIds.length > 0) {
      ...
      if (usersError) {
        console.error(
          'Error obteniendo destinatarios:',
          usersError
        );
    ...
    console.error(
      'Error GET /friends/requests/sent:',
      error
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        'Error POST /friends/requests/:id/accept:',
        error
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        'Error POST /friends/requests/:id/reject:',
        error
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        'Error POST /friends/requests/:id/cancel:',
        error
      );
    ...
    if (friendIds.length > 0) {
      ...
      if (usersError) {
        ...
        return res.status(500).json({
          success: false,
          error:
            'No se pudieron obtener los datos de los amigos'
        });
      }
